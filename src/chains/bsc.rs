//! Binance Smart Chain (BSC) indexer
//!
//! Uses the same contract ABI as Ethereum since BSC is EVM-compatible.
...
impl BscIndexer {
    /// Create a new BSC indexer
    pub async fn new(config: &BscConfig) -> Result<Self, ChainError> {
        ...
        info!(
            "BSC indexer initialized: contract={}, chain_id={}",
...
#[async_trait]
impl ChainIndexer for BscIndexer {
    fn chain(&self) -> Chain {
        Chain::BSC
    }
    ...
    async fn watch(&self) -> Result<(), ChainError> {
        info!("Starting BSC block watcher");
        
        let mut last_block = self.get_block_height().await?;
        
        loop {
            tokio::time::sleep(tokio::time::Duration::from_secs(3)).await; // 3s block time
            ...
            if current_block > last_block {
                ...
                if !result.messages.is_empty() {
                    info!(
                        "BSC: Found {} messages in blocks {} to {}",
                        result.messages.len(),
                        last_block + 1,
                        current_block
                    );
